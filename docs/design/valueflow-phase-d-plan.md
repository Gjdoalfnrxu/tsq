...
**Status:** plan only. No code. Branch `design/valueflow-phase-d`,
worktree `/tmp/tsq-valueflow-phase-d`.
**Author:** Planky.
**Date:** 2026-04-19.
...
### 4.5 Aggregate keep-or-revert (PR7 / final)

The full layer is **kept** if and only if, summed across all migrated
bridges:

- ≥ 4 of 6 bridges merged green on all corpora.
- React (PR7) is green on Mastodon — non-negotiable; React is the
  proof-of-concept and the largest line-deletion win.
- Net lines deleted across `bridge/` ≥ 400.
- At least one cap-hit eliminated on Mastodon vs the pre-Phase-C
  baseline.

If any of the above fails: **revert PR7**, keep the layer in tree
(`bridge/tsq_valueflow.qll`) as opt-in, file a postmortem in the wiki
under `Wiki/Tech/tsq-valueflow-phase-d-postmortem.md`, and move
follow-up to a Phase E redesign.
